...
# shellcheck disable=SC2164
# shellcheck disable=SC1083
helm show values mettle/backend-generic > values.yaml --version "$(helm dependency list | awk '{ print $2 }'  | tail -n +2)"
...
yq -e -i '.application.image.repository = "eu.gcr.io/eevee-bank/test"' values.yaml
# Put the upstream values under the dependency name, so the values apply to the subchart
yq -e -i '{"backend-generic": .}' values.yaml
...
# shellcheck disable=SC2164
# shellcheck disable=SC1083
cp -r test_app_chart/charts .
# shellcheck disable=SC2164
# shellcheck disable=SC1083
cp -r test_app_chart/.github .
# shellcheck disable=SC2164
# shellcheck disable=SC1083
rm -rf test_app_chart

echo "App chart created, see usage info in charts/test/README.md"